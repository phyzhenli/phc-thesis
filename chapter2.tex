...
% 。
% \else
\footnote{https://www.quora.com/How-does-Booths-algorithm-work/answer/Raymond-Paseman}。
% \fi
...
，
\else
\footnote{https://blog.csdn.net/mydreamongo/article/details/8863502}，
\fi
...
，
\else
\footnote{https://zhuanlan.zhihu.com/p/343133392}，
\fi
...
，
\else
\footnote{https://picture.iczhiku.com/resource/eetop/whKDFaTwqTZOkCxn.pdf}，
\fi
...
。
\else
\footnote{https://blog.csdn.net/qq\_26707507/article/details/106146612}。
\fi
...
% ：
% \else
\footnote{https://en.wikipedia.org/wiki/Dadda\_multiplier}：
% \fi
\begin{itemize}
...
，
\else
\footnote{https://zhuanlan.zhihu.com/p/378267920}，
\fi
...
但是，与RCA相比，CSKA并没有明显的性能改进（如图\ref{EM:Fig:CSKA_4bit}中的关键路径$\tau_{\text{crit}}$同样需要经过4个全加器）。因此，当位宽较大时，采用分块CSKA（Block CSKA）的办法才能取得较为显著的速度增益。若一个$n$比特的分块CSKA有$m$块CSKA，每块CSKA的位宽均为$\dfrac{n}{m}$，则该分块CSKA被称为固定大小分块CSKA\footnote{https://en.wikipedia.org/wiki/Carry-skip\_adder}。
\begin{figure}[!htb]
    \centering
...
，
\else
\footnote{https://kexue.fm/archives/7991}，
\fi
