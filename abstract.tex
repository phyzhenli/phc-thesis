\begin{abstract}
    ...
    对方法进行了大量的实验，结果表明，均匀分布下生成的8比特无符号乘法器大幅优于国际前沿工作，针对采用8比特无符号数量化的LeNet、AlexNet和VGG16生成的乘法器在精度损失不超过0.01\%的情况下实现了26.4\%-47.6\%的性能收益，面向自适应滤波器生成的16位补码有符号乘法器在PSNR损失可以忽略的情况下实现了27.1\%的提升，基于32比特半正态分布的实验结果表明提出的方法对大位宽乘法器同样有效。
